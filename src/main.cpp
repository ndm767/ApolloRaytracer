...
int main(int argc, char *argv[]) {
    ...
    ("c,usecpu", "forces using only the CPU", cxxopts::value<bool>()->default_value("false"))
    ...
    bool useCPU = results["usecpu"].as<bool>();

    ApolloRaytracer apollo(scene, width, height, res, depth, samples, useCPU);
    apollo.run();
    return 0;
}